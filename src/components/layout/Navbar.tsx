...
export const Navbar: React.FC<NavbarProps> = ({ onOpenService, onCloseService }) => {
  ...
  const [mobileServicesOpen, setMobileServicesOpen] = useState(false); // Add this state
        ...
        <div className="space-y-6 pb-20">
          ...
          <div>
            <button
              ...
              className="flex items-center justify-between w-full mobile-link text-left group"
            >
              <span>Servicios</span>
              <ChevronDown
                size={24}
            ...
            <div className={`overflow-hidden transition-all duration-300 ${mobileServicesOpen ? 'max-h-[1000px] opacity-100 mt-4' : 'max-h-0 opacity-0'}`}>
              <div className="border-l-2 border-white/10 pl-6 py-2 space-y-4">
                {servicesData.map(service => (
                  <button
                    key={service.id}
                    onClick={() => handleServiceClick(service.id)}
                    className="block text-gray-300 text-lg hover:text-white transition-colors text-left w-full py-1 font-light"
                  >
                    {service.title}
                  </button>
                ))}
  ...
  const handleMouseLeave = () => {
            ...
            >
              <div className="container mx-auto px-4 md:px-6 lg:px-12 flex justify-between items-center relative">
                {/* Logo */}
                <div className="cursor-pointer flex items-center gap-0.5 relative z-50 shrink-0" onClick={() => scrollToSection(SectionId.HOME)}>
                  {/* Icon */}
                  <img
